...
| PF15 | greedy | this commit | `ChunkMesher` merges cube and liquid faces greedily (per direction and slice, as wide as a row allows along u, then whole rows along v), a merge allowed along a direction only where the corners' AO does not change along it, so the interpolation is the unit faces'; the key is block, sky and block light, the four AO corners and the lowered liquid top. The per-voxel colour variation left the vertex colour for the terrain shader (`VoxelTint`, the same 32-bit hash as `ChunkMesher.voxelTint`, of the cell a tenth of a block behind the face); the unlit `glow` surface keeps it baked and unmerged. **The example's `orbit:6` window** (a probe meshing the benchmark's 13 × 13 chunks, its count equal to the benchmark's `faces`): **faces 185,130 → 114,193** (solid 165,431 → 113,902, −31%; liquid 19,699 → 291), vertices 740,520 → 456,772; the mesh job 3.67 → 4.11 ms a chunk (JIT). Solid merges less than a flat world would: hills put AO on most edges, and the grass/dirt/stone and ore ids split the sides. A frame of `orbit:6` against the baked variation (release, Mac): 97.7% of the pixels identical, 99.7% within 2 levels, the rest (up to 37) on cells' edges, where multisampling now shades one cell's variation. |

**Where the work stopped (2026-09-25, PF14 closed on both platforms).** Last commit: this
one (PF14's regions of 4, measured and not adopted), over `d797692` (PF14's phone A/B) and
`3577f2a` (the runner says why a phone run died). PF14 itself is `2f89a05` (`a630524` on `dev`,
the merge of `main`, has its tree). Next step: **PF15** (greedy meshing, the tint in the
shader), then PF13, both judged on the Mac by `--trace`'s GPU ms a frame (the Mac is
GPU-bound at `orbit:6`: 9.99 ms a frame against 8.3) and on the phone by encode and fps;
PF9 only after a trace shows GC in the UI thread. `KL-008` (the driver crash in the first
two seconds) stayed away in 84 launches since; if a run dies, the runner now prints the
exit reason and the crash log, and the tombstone is in `adb shell dumpsys dropbox --print
SYSTEM_TOMBSTONE`. After PF14 the phone runs `mobs:6` at 79–91 fps (encode p50 4.5 ms, UI
p50 6.2), `orbit:6` at 112–116, `fly:6` at the display's 117; the Mac at ~102, GPU-bound as
`orbit:6` is (its ceiling is the terrain's: PF15, PF13). **The phone's CPU clock is a closed lead:** four busy
...
`--repeat 1` and launches again, `--no-build`, only the scenarios a failed call left.
