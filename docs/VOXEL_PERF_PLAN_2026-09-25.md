...
**Where the work stopped (2026-09-25).** Last commit: the PF6 search budget's numbers
(docs). Next step: **PF7** (mob bodies), then PF14; PF9 only after a trace shows GC in the
UI thread (§Progress, PF9 measured). With 40 creatures a step is no longer the wall on
either device: step p99 1.15 ms on the Mac, 4.5 on the phone, the loop at 60 steps a
second. The phone runs `mobs:6` at 58 fps because its UI thread spends 12.4 ms a frame at
p50, 10.7 of them encoding the scene against 4.9 at `orbit:6` without creatures: the
creatures' bodies cost ~6 ms of encoding, PF7's target. The Mac runs it at ~102, GPU-bound
as `orbit:6` is without creatures (~100): its ceiling is the terrain's (PF15, PF13).
Judge a step by `stepMs`, not `simMs`. An A/B of anything with creatures takes `d34acab`
or later as its before (the Wander fix changed the load). A search still spends its 400
nodes toward an unreachable goal (0.3 ms on the Mac); no longer in the p99, so left. A phone run
