...
**Where the work stopped (2026-09-25, after PF7).** Last commit: PF7's numbers (docs),
over `e770ef0` (the creatures of a species share their meshes). Next step: **PF14** (chunk
...
With 40 creatures the phone runs `mobs:6` at 68 fps (encode p50 6.2 ms, UI p50 9.5), the Mac
at ~102, GPU-bound as `orbit:6` is (its ceiling is the terrain's: PF15, PF13). **New lead,
not yet a step: the phone's CPU clock.** With four busy loops holding the CPU up, the same
build runs `mobs:6` at 92–99 fps with encode 3.6 ms and UI p50 ~5: Android's governor lowers
the clock when a frame's work shrinks, so the frame rate is now below what the phone can do.
Android's Dynamic Performance Framework (`PerformanceHintManager` hint sessions: the
frame's target and actual durations reported to the governor) exists for exactly this; it
would be a platform channel in `voxel_game`'s Android side. Worth measuring before PF14,
since every UI-thread step is judged through this clock on the phone. The same effect
reads on the Mac as a slower step when the frame gets cheaper: judge a step's cost with
the CPU held busy (`yes > /dev/null` ×2) when the A/B moves the UI thread. Judge a step by
