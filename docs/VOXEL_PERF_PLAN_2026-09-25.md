...
| PF14 | regions | this commit | `VoxelChunkView` draws chunks in 2 × 2 regions: one node a region, one geometry a surface, the chunk offsets baked into the positions (`MergedSurface`), 16-bit indices while they fit, bounds taken during the copy; a chunk's apply or removal rebuilds its region from the surfaces the view keeps. flutter_scene's profile, `orbit:6`: colour-pass **draws 85–101 → 39–40** on the Mac, encode 326–454 → 164–189 µs; on the phone the colour pass cost 3.7–4.0 ms for 95–99 draws before (~31 µs a draw, ~4 on the Mac). A/B against `70ca8dc` (the tree of `6d990fb`), three rounds alternated, 120 Hz, Mac (`docs/perf/pf14_mac120_ab_{70ca8dc,regions}.jsonl`), no overlap between runs: **encode p50 orbit 0.63 → 0.31 ms** (0.62–0.64 → 0.30–0.31), mobs 1.10 → 0.75, fly 0.58 → 0.29; UI p50 −29 to −44%, UI p99 orbit 1.91 → 0.90, mobs 4.44 → 2.20; fps unchanged (GPU-bound); the costs: **fly UI p99 2.29 → 2.89** (a region's rebuild when a chunk streams in) and **RSS +34 to +53 MB** (the kept surfaces). Galaxy S24, phone preset, **partial** (`docs/perf/pf14_s24_phone_ab_*.jsonl`: the runner lost a run with no result line three times, on both sides, so 2 rounds after and 1–2 before): mobs **69.7 · 77.5 → 85.9 · 89.4 fps**, encode p50 6.94 · 5.49 → 4.74 · 4.57; orbit **83.4 → 112.8 · 117.1**, encode 5.91 → 3.86 · 3.83; fly 102.4 → 118.1 · 117.7, encode 4.68 → 3.39 · 3.49; RSS +8 to +39 MB. |

**Where the work stopped (2026-09-25, PF14 committed, its phone A/B partial).** Last commit:
PF14 (chunk regions, `voxel_scene`), over the ADPF probe (`6d990fb`, a rewrite of `70ca8dc` with the same tree). Next step: **finish PF14's phone A/B** (three full rounds against `70ca8dc`, `mobs:6,orbit:6,fly:6`) and write its numbers into the PF14 row; the runner lost a run with no result line three times (`mobs` after `orbit` twice, then `orbit` after `mobs`, on both sides; logcat held only the engine's start lines): find why first (a crash, the process not yet gone at the next `am start -S`?). Then judge whether `regionChunks` 4 buys more encode than it costs in `fly`'s UI p99 and RSS, then PF15 / PF13 (the Mac's GPU); PF9 only after a trace shows GC in the UI thread.
Before PF14, with 40 creatures, the phone ran `mobs:6` at 67–82 fps (encode p50 6.2 ms, UI p50 9.5; the
